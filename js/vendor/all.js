...
if ($('#seolinechart2').length) {
    ...
    var chart = new Chart(ctx, {
        ...
        // The data for our dataset
        data: {
            labels: ["Janeiro", "February", "March", "April", "May", "June", "July", "January", "February", "March", "April", "May"],
            datasets: [{
                label: "Quant.",
...
if ($('#user-statistics').length) {
    ...
    function datashora(dat){
        var dia = dat.split(' ');
        return String(dia[0])
        
    }
    
    function processDataConsumo(arquivo) {
        var x = [];
        var allLines = arquivo.split('\n');
	    if(allLines.length > 0) {
            var cont = 0;
            var volume = 0;
		    for(var i=2; i< allLines.length; i++) {
                for(var i=2; i< allLines.length ; i++) {
                    var line = allLines[i-1].split(',');
                    var line1 = allLines[i].split(',');
                    if(cont == 0){
                        volume = volume + parseInt(line[1]);
                    }
                    if (datashora(String(line[0])) == datashora(String(line1[0]))){
                        volume = volume + parseInt(line1[1]);
                        cont++;
                    ...
                    {    
                        ...
                        x.push({"date": datashora(String(line[0])), "value": volume});
                        volume = 0;
                    ...
                    if(i == allLines.length-1){
                        if (datashora(String(line[0])) != datashora(String(line1[0]))){
                            ...
                            x.push({"date": datashora(String(line[0])), "value": parseInt(line1[1])});
    ...
    function processDataDetalhado(arquivo) {
        var x = [];
        var allLines = arquivo.split('\n');
	    if(allLines.length > 0) {
            var cont = 0;
		    ...
		    for(var i=2; i< allLines.length ; i++) {
                ...
                {    
                    x.push({
                        "date": datashora(String(line[0])), 
                        "market1": volume, 
                        "market2": parseInt(line[2]), 
                        "sales1": parseInt(line[3]), 
                        "sales2": parseInt(cont+1)
                    });
                    volume = 0;
                }
                if(i == allLines.length-1){
                    if (datashora(String(line[0])) != datashora(String(line1[0]))){
                        x.push({
                            "date": datashora(String(line1[0])), 
                            "market1": parseInt(line1[1]), 
                            "market2": parseInt(line1[2]), 
                            "sales1": parseInt(line1[3]), 
                            "sales2": parseInt(cont+1)
                        });
                    }else{
    ...
    $.ajax({
        ...
        success: function(data) {
            consumo = processDataConsumo(data);
            detalhado = processDataDetalhado(data);
        }
    });
...
(function($) {
    ...
    ==================================*/
    $('.settings-btn, .offset-close').on('-+-', function() {
